...
public:
  ...
  {
    _d.r = _d.g = _d.b = intensity;
    _d.a = alpha;
  }

  Color(int intensity, int alpha)
  {
    _d.r = _d.g = _d.b = (intensity / 255.f);
    _d.a = alpha / 255.f;
  }
  ...
  bool operator!=(const Color& c) { return !(c.a() == a() && c.r() == r() && c.g() == g() && c.b() == b()); }
  ...
  Color operator*(float m) const { return Color(r() * m, g() * m, b() * m, a() * m); }
...
public:
  ...
  bool operator<=(const Vector2<T>& o) const { return (x < o.x || math::isEqual(x, o.x)) || (math::isEqual(x, o.x) && (y < o.y || math::isEqual(y, o.y))); }

  bool positive() const { return x >= 0 && y >= 0; }

  bool lessOrEq(const Vector2<T>& o) const { return (x < o.x || math::isEqual(x, o.x)) && (y < o.y || math::isEqual(y, o.y)); }
  ...
  bool IsEqual(const Vector2<T>& o, float tolerance) const { return math::isEqual<T>(x, o.x, tolerance) && math::isEqual<T>(y, o.y, tolerance); }
  ...
  T dotProduct(const Vector2<T>& o) const { return x * o.x + y * o.y; }
  ...
  float getDistanceFrom(const Vector2<T>& o) const { return Vector2<T>(x - o.x, y - o.y).getLength(); }

  //! Returns squared distance from ano point.
  /** Here, the vector is interpreted as a point in 2-dimensional space.
  \param o o vector to measure from.
  \return Squared distance from o point. */
  T getDistanceFromSQ(const Vector2<T>& o) const { return Vector2<T>(x - o.x, y - o.y).getLengthSQ(); }
  ...
  Vector2<T> getInterpolated_quadratic(const Vector2<T>& v2, const Vector2<T>& v3, const T d) const
  {
    ...
    return Vector2<T>((T)(x * mul0 + v2.x * mul1 + v3.x * mul2), (T)(y * mul0 + v2.y * mul1 + v3.y * mul2));
  }

  Vector2<T> cmax(T rx, T ry) const { return Vector2<T>(std::max(x, rx), std::max(y, ry)); }

  Vector2<T> cmax(const Vector2<T>& o) const { return Vector2<T>(std::max(x, o.x), std::max(y, o.y)); }

  Vector2<T> cmin(const Vector2<T>& o) const { return Vector2<T>(std::min(x, o.x), std::min(y, o.y)); }
  ...
  */
  ...
  template<class B>
  inline Vector2<B> cast() const
  {
    return Vector2<B>(static_cast<B>(x), static_cast<B>(y));
  }
