...
void ImageView::drawImageBorder(SDL_Renderer* renderer, const SDL_Point& ap) const
{
  ...
  if (r.x1 > wr.x1)
    SDL_RenderDrawLine(renderer, r.x1, r.y1, r.x1, r.y2 - 1);
  if (r.y1 > wr.y1)
    SDL_RenderDrawLine(renderer, r.x1, r.y1, r.x2 - 1, r.y1);
  if (r.x2 < wr.x2)
    SDL_RenderDrawLine(renderer, r.x2, r.y1, r.x2, r.y2 - 1);
  if (r.y2 < wr.y2)
    SDL_RenderDrawLine(renderer, r.x1, r.y2, r.x2 - 1, r.y2);
}
...
void ImageView::drawPixelGrid(SDL_Renderer* renderer, const Vector2f& upperLeftCorner, const Vector2f& lowerRightCorner, const float stride)
{
  SDL_SetRenderDrawColor(renderer, 255, 255, 255, 255);
  ...
  {
    SDL_RenderDrawLine(renderer, std::floor(currentX), std::floor(upperLeftCorner.y), std::floor(currentX), std::floor(lowerRightCorner.y));
    currentX += stride;
  }
  ...
  {
    SDL_RenderDrawLine(renderer, std::floor(upperLeftCorner.x), std::floor(currentY), std::floor(lowerRightCorner.x), std::floor(currentY));
    currentY += stride;
  }
}
...
void ImageView::writePixelInfo(SDL_Renderer* renderer, const Vector2f& cellPosition, const Vector2i& pixel, const float stride) const
{
