...
void TabHeader::TabButton::calculateVisibleString(SDL_Renderer* renderer)
{
  ...
  std::string displayedText = mHeader->theme()->breakText(renderer, mLabel.c_str(), "sans", mHeader->fontSize(), mSize.x - 10);
...
void TabHeader::TabButton::drawActiveBorderAt(SDL_Renderer* renderer, const Vector2i& position, float offset, const Color& color)
{
  int xPos = position.x;
  int yPos = position.y;
  int width = mSize.x;
...
void TabHeader::TabButton::drawInactiveBorderAt(SDL_Renderer* renderer, const Vector2i& position, float offset, const Color& color)
{
  int xPos = position.x;
  int yPos = position.y;
  int width = mSize.x;
...
{
  auto element = std::find_if(mTabButtons.begin(), mTabButtons.end(), [&](const TabButton& tb) { return label == tb.label(); });
...
{
  auto it = std::find_if(mTabButtons.begin(), mTabButtons.end(), [&](const TabButton& tb) { return label == tb.label(); });
...
std::pair<Vector2i, Vector2i> TabHeader::visibleButtonArea() const
{
  ...
  auto width = std::accumulate(visibleBegin(), visibleEnd(), theme()->mTabControlWidth, [](int acc, const TabButton& tb) { return acc + tb.size().x; });
...
std::pair<Vector2i, Vector2i> TabHeader::activeButtonArea() const
{
  ...
  auto width = std::accumulate(visibleBegin(), activeIterator(), theme()->mTabControlWidth, [](int acc, const TabButton& tb) { return acc + tb.size().x; });
...
bool TabHeader::mouseButtonEvent(const Vector2i& p, int button, bool down, int modifiers)
{
  ...
  if (button == SDL_BUTTON_LEFT && down)
  {
    ...
    {
      case ClickLocation::LeftControls: onArrowLeft(); return true;
      case ClickLocation::RightControls: onArrowRight(); return true;
      case ClickLocation::TabButtons:
        auto first = visibleBegin();
        auto last = visibleEnd();
        int currentPosition = theme()->mTabControlWidth;
        int endPosition = p.x;
        auto firstInvisible = std::find_if(first, last, [&currentPosition, endPosition](const TabButton& tb) {
...
void TabHeader::calculateVisibleEnd()
{
  ...
  auto firstInvisible = std::find_if(first, last, [&currentPosition, lastPosition](const TabButton& tb) {
    currentPosition += tb.size().x;
...
void TabHeader::drawControls(SDL_Renderer* renderer)
{
  ...
  if (_lastLeftActive != lactive || _lastRightActive != ractive)
  {
    ...
    if (_lastLeftActive != lactive)
    {
      ...
      mTheme->getTexAndRectUtf8(renderer, _leftIcon, 0, 0, iconLeft.data(), "icons", ih, lactive ? mTheme->mTextColor : mTheme->mButtonGradientBotPushed);
    }

    if (_lastRightActive != ractive)
    {
      auto iconRight = utf8(ENTYPO_ICON_RIGHT_BOLD);
      mTheme->getTexAndRectUtf8(renderer, _rightIcon, 0, 0, iconRight.data(), "icons", ih, ractive ? mTheme->mTextColor : mTheme->mButtonGradientBotPushed);
    }
