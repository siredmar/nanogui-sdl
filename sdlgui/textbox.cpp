...
struct TextBox::AsyncTexture
{
  ...
  void load(TextBox* ptr, bool editable, bool focused, bool validFormat, bool outside)
  {
    ...
    std::thread tgr([=]() {
      ...
      NVGpaint bg = nvgBoxGradient(ctx, dx + 1, dy + 1 + 1.0f, ww - 2, hh - 2, 3, 4, Color(255, 128).toNvgColor(), Color(32, 32).toNvgColor());
      NVGpaint fg1 = nvgBoxGradient(ctx, dx + 1, dy + 1 + 1.0f, ww - 2, hh - 2, 3, 4, Color(150, 32).toNvgColor(), Color(32, 32).toNvgColor());
      NVGpaint fg2 = nvgBoxGradient(ctx, dx + 1, dy + 1 + 1.0f, ww - 2, hh - 2, 3, 4, nvgRGBA(255, 0, 0, 100), nvgRGBA(255, 0, 0, 50));
...
void TextBox::drawBody(SDL_Renderer* renderer)
{
  ...
  int id = (mEditable ? 0x1 : 0) + (focused() ? 0x2 : 0) + (mValidFormat ? 0x4 : 0) + (outside ? 0x8 : 0);
...
void TextBox::draw(SDL_Renderer* renderer)
{
  ...
  */
  ...
  switch (mAlignment)
  {
    case Alignment::Left: drawPos.x = getAbsoluteLeft() + xSpacing + spinArrowsWidth; break;
    case Alignment::Right: drawPos.x = getAbsoluteLeft() + mSize.x - _captionTex.w() - unitWidth - xSpacing; break;
  ...
  else
  {
    ...
    if (mCursorPos > -1)
    {
      ...
      if (caretLastTickCount % 1000 < 500)
      {
        ...
        SDL_SetRenderDrawColor(renderer, c.r, c.g, c.b, c.a);
        SDL_RenderDrawLine(renderer, oldDrawPos.x + caretx, oldDrawPos.y + 4, oldDrawPos.x + caretx, oldDrawPos.y + lineh - 3);
      }
    }
  }
}
